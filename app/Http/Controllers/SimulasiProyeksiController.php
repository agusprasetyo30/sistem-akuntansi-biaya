...
//                                    ->sum('total_biaya');
                                ...
                                $res_biaya_perton = $collection_input_temp
                                    ...
                                    ->where('kategori', '!=', 0)
                                    ->whereIn('no', [1, 2, 3, 4])
                                    ->where('asumsi_umum_id', '=', $periode)->sum('biaya_perton');
                                $res_total_biaya = $collection_input_temp
                                    ->where('product_code', $data_product)
                                    ->where('kategori', '!=', 0)
                                    ->whereIn('no', [1, 2, 3, 4])
                                    ->where('asumsi_umum_id', '=', $periode)->sum('total_biaya');

//                                if ($data_product == '2000002'){
//                                    dd($res_biaya_perton, $res_total_biaya);
//                                }
...
//                            dd($data_version, $data_plant, $asumsi->month_year);
