let table = function () {
	document.getElementById('table-wrapper').innerHTML = ewpTable({
        ...
        column : [
            {name : 'NO', width : '5'},
            {name : 'NAMA', width : '20'},
            {name : 'AWAL PERIODE', width : '5'},
            {name : 'AKHIR PERIODE', width : '5'},
	...
	ewpDatatables({
		...
		column : [
			...
			{col : 'periode_name', mid : false, mod : false},
			{col : 'awal_periode', mid : false, mod : {
                'aTargets': [2],
                'mRender': function (data, type, full) {
					return helpDateFormat(full.awal_periode, 'mi');
                }
            }},
			{col : 'akhir_periode', mid : true, mod : {
                'aTargets': [3],
                'mRender': function (data, type, full) {
					return helpDateFormat(full.akhir_periode, 'mi');
                }
            }},
			...
			{col : 'id', mid : true, mod : {
                ...
                'mRender': function (data, type, full) {
					let btn_action = `
                    <a href="javascript:;" onclick="detail('` + full.id + `', this)" id="btn-detail-` + full.id + `" class="btn bg-info-transparent"><i class="fe fe-info"></i></a>
                    <a href="javascript:;" onclick="edit('` + full.id + `', this)" id="btn-ubah-` + full.id + `" class="btn bg-warning-transparent"><i class="fe fe-edit"></i></a>
                    <a href="javascript:;" onclick="trash('` + full.id + `', this)" id="btn-hapus-` + full.id + `" class="btn bg-danger-transparent"><i class="fe fe fe-trash"></i></a>`;
...
let store = function (url, edit = false, modal = '#modal') {
    ...
    formData.append('periode_name', $('[name="periode_name"]').val())
    formData.append('awal_periode', $('[name="awal_periode"]').val())
    formData.append('akhir_periode', $('[name="akhir_periode"]').val())
